...
def get_results_row(exec_time):
    ...
    result_Noise_type=args.noise_type
    result_Noise_fraction=args.noise_rate
    if args.dataset == 'cifar10':
        result_Neural_network = 'CNN_small'
        
    if args.dataset == 'cifar100':
        result_Neural_network = 'CNN'
    if args.dataset=='imagenet_tiny':
        result_Neural_network = 'PreActResNet18'

    result_denoising_algorithm=args.model_type
    result_Epochs = args.n_epoch
    ep_logs = pd.read_csv(txtfile, delimiter=' ')
    result_train_acc = max(max(ep_logs["train_acc1"]),max(ep_logs["train_acc2"]))
    result_test_acc = max(max(ep_logs["test_acc1"]),max(ep_logs["test_acc2"]))
    return [result_Dataset, result_Noise_type,result_Noise_fraction,result_Neural_network, result_denoising_algorithm, result_Epochs, result_train_acc, result_test_acc, exec_time]
...
if __name__ == '__main__':
    ...
    if (not os.path.exists('./results.csv')):
        with open("./results.csv", 'a') as results_csv:
            ...
            csvwriter.writerow(['Dataset',	'Noise type','Noise fraction','Neural network', 'Denoising Algorithm',
